...
class SwinTransformer(torch.nn.Module):
    ...
    def forward(self, X):
        h = self.slice1(X)
        h_relu1 = h.permute(0,3,1,2)
        h = self.slice2(h)
        h_relu2 = h.permute(0,3,1,2)
        h = self.slice3(h)
        h_relu3 = h.permute(0,3,1,2)
        h = self.slice4(h)
        h_relu4 = h.permute(0,3,1,2)
        swin_outputs = namedtuple("AlexnetOutputs", ['relu1', 'relu2', 'relu3', 'relu4'])
        out = swin_outputs(h_relu1, h_relu2, h_relu3, h_relu4)
